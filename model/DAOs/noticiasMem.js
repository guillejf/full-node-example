...
class NoticiasMemDAO extends NoticiasBaseDAO {
  ...
  guardarNoticia = async (noticia) => {
    try {
      ...
      let noticiaGuardada = { ...noticia, _id, fyh };
  ...
  actualizarNoticia = async (_id, noticia) => {
    try {
      let fyh = new Date().toLocaleString();
      let noticiaNew = { ...noticia, _id, fyh };
      ...
      indice >= 0
        ? this.noticias.splice(indice, l, noticiaActualizada)
  ...
  borrarNoticia = async (_id) => {
    try {
      let indice = this.getIndex(_id, this.noticias);
      if ((indice = -1)) throw "Noticia no encontrada";
